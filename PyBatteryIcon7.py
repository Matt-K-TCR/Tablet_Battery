...
class BatteryIcon:
    ...
    def update_battery_level(self):
        ...
        if battery:
            ...
            text_color = "white" if percent > 50 else "black"  # White for green, black for orange/red

            # Update battery fill
            self.canvas.coords(self.battery_fill, 6, 8, 6 + percent * 0.44, 22)
            self.canvas.itemconfig(self.battery_fill, fill=color)

            # Add shadow effect for white text
            if text_color == "white":
                # Create shadow by drawing the text in black first (slightly offset)
                self.canvas.itemconfig(self.battery_text, text=f"{percent}%", fill="black")
                self.canvas.coords(self.battery_text, 31, 16)  # Adjust the position for shadow
                self.canvas.create_text(30, 15, text=f"{percent}%", fill="white", font=("Arial", 6, "bold"))
            else:
                # For black text, just display it normally
                self.canvas.itemconfig(self.battery_text, text=f"{percent}%", fill=text_color)
                self.canvas.coords(self.battery_text, 30, 15)
